...
package body Day.Test is
  ...
  procedure Test_Part2 (T : in out AUnit.Test_Cases.Test_Case'Class) is
    pragma Unreferenced (T);
    v : constant Adaptors.Vector := load_file("test1.txt");
    t1 : constant Long_Integer:= total_arrangments(v);
    v2 : constant Adaptors.Vector := load_file("test2.txt");
    t2 : constant Long_Integer:= total_arrangments(v2);
  begin
    Assert(t1 = 8, "Wrong number, expected 8, got" & Long_Integer'IMAGE(t1));
    Assert(t2 = 19208, "Wrong number, expected 19208, got" & Long_Integer'IMAGE(t2));
  end Test_Part2;
